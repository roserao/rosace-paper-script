...
### load rosace file
load("data/plot_rosette_MET/rosace_MET1.RData")
load("data/plot_rosette_MET/rosette_MET.RData")
sdir <- "data/plot_rosette_MET/"


df_score <- data.frame(score = rosace@scores[[1]]@score$estimate, type = "real")
df_score_var2 <- 
  cbind(ExtractVarScore(rosace, name = "MET1_SLR"),
...
p1 <- PlotDisp(object = rosace, name = "MET1_1",
               ctrl.col = "type", ctrl.name = "synonymous")
p1 <- p1 + theme_cowplot() + 
  geom_hline(yintercept = 1.3426, color = "red", linetype = "dashed") +
  labs(y = "estimated eta", x = "number of synonymous variants randomly chosen")
save_plot(p1, file = file.path(sdir, "eta.png"), base_width = 5, base_height = 4)
...
cfg.clean <- CreateConfig(rosette,
                          n.sim = 2, save.sim = sdir, type.sim = "growth",
                          n.rep = 3, n.round = 3,
